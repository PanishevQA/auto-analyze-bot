...
class Questionnaire(StatesGroup):
    ...
    body_type = State(); description = State(); photos = State(); confirmation = State()
...
async def drom_url(message: Message, state: FSMContext) -> None:
    ...
    await state.update_data(source_url=source_data.source_url); await ask_make(message, state)
...
async def save_text(message: Message, state: FSMContext, key: str, next_state: State,
    ...
    await state.update_data(**{key: value}); await state.set_state(next_state); await message.answer(question)
...
async def year(message, state):
    ...
    await state.update_data(year=value); await state.set_state(Questionnaire.generation); await message.answer("Поколение или /skip:")
...
async def mileage(message, state):
    ...
    await state.update_data(mileage_km=value); await state.set_state(Questionnaire.price); await message.answer("Цена продавца, ₽:")
@router.message(Questionnaire.price)
async def price(message, state):
    try: value=validate_price(message.text or "")
    except ValueError as error: await message.answer(f"❌ {error}"); return
    await state.update_data(asking_price_rub=value); await state.set_state(Questionnaire.engine_volume); await message.answer("Объём двигателя, например 1.6, или /skip:")
...
async def engine_volume(message, state):
    ...
    await state.update_data(engine_volume_l=value); await state.set_state(Questionnaire.fuel_type)
...
async def fuel(callback,state):
    await state.update_data(fuel_type=callback.data.split(":",1)[1]); await state.set_state(Questionnaire.horsepower)
...
async def horsepower(message,state):
    ...
    await state.update_data(horsepower=value); await state.set_state(Questionnaire.transmission)
...
async def transmission(callback,state):
    await state.update_data(transmission=callback.data.split(":",1)[1]); await state.set_state(Questionnaire.drive)
...
async def drive(callback,state):
    await state.update_data(drive=callback.data.split(":",1)[1]); await state.set_state(Questionnaire.body_type)
    await callback.message.answer("Тип кузова или /skip:"); await callback.answer()
@router.message(Questionnaire.body_type)
async def body(message,state): await save_text(message,state,"body_type",Questionnaire.description,"Описание продавца или /skip:",True)
...
async def photos_control(callback,state):
    ...
    elif action=="done": await show_confirmation(callback.message,state)
...
async def edit_menu(callback,state):
    fields=[("Марка","make"),("Модель","model"),("Год","year"),("Поколение","generation"),
        ...
        ("КПП","transmission"),("Описание","description"),("Фотографии","photos")]
...
async def edit_field(callback,state):
    ...
    mapping={"make":Questionnaire.make,"model":Questionnaire.model,"year":Questionnaire.year,
        ...
        "transmission":Questionnaire.transmission,"description":Questionnaire.description,"photos":Questionnaire.photos}
    await state.set_state(mapping[key]); await callback.message.answer("Введите новое значение (дальнейшие ответы можно подтвердить заново):")
    await callback.answer()
