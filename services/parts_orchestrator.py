...
async def build_parts_provider(settings, yandex_client=None,agent=None):
    ...
    browser=BrowserPartsProvider(settings.drom_baza_start_url,headless=settings.parts_browser_headless,
        ...
        min_offers=settings.parts_min_matched_offers,match_confidence=float(settings.parts_match_confidence),agent=agent)
...
class PartsSearchOrchestrator:
    ...
    async def estimate(self, vehicle: VehicleSpec, defects: list[VisibleDefect], repairs: RepairEstimate,
        ...
        requested_condition=condition or self.default_condition
        ...
        for defect in defects:
            ...
            try: plan=await self.agent.create_plan(vehicle,defect.part,side,position,requested_condition)
            except Exception as error:
                logger.warning("Yandex parts plan unavailable defect_id=%s error=%s",defect.defect_id,type(error).__name__)
                plan=self.agent.build_plan(vehicle,defect.part,side,position,requested_condition)
            ...
            query=PartSearchQuery(defect_id=defect.defect_id,vin=vehicle.vin,make=vehicle.make,model=vehicle.model,year=vehicle.year,
                ...
                search_phrase=plan.query,quantity=item.quantity,region=vehicle.region,condition=requested_condition)
