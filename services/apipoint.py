...
class APIpointClient:
    ...
    async def estimate(self, vehicle: VehicleSpec, analysis_id: str | None = None) -> MarketEstimate:
        ...
        for alias in ("avgcarprice", "carprices"):
            ...
            try:
                ...
                return result
            except (httpx.TimeoutException, httpx.HTTPError, InvalidMarketResponse) as error:
    ...
    def _body(self, vehicle: VehicleSpec, alias: str) -> dict[str, Any]:
        ...
        if alias == "avgcarprice":
            ...
            # В доступном контракте подтверждено только значение MANUAL; остальные не выдумываем.
            transmission = "MANUAL" if vehicle.transmission == "MANUAL" else None
        ...
        for attempt in range(2):
            ...
            try:
                ...
                if 400 <= response.status_code < 500:
                    response.raise_for_status()  # обычный 4xx без retry
                try:
                    value = response.json()
    ...
    def _normalize(self, payload: dict[str, Any], alias: str, *,
        ...
        if payload.get("status") not in (None, True, "success", "ok", 1):
            raise InvalidMarketResponse("Верхнеуровневый status неуспешен")
        try:
            block = payload["result"][alias]
            if block.get("error") is True:
                raise InvalidMarketResponse(str(block.get("error_msg") or "APIpoint nested error"))
            result = block["result"]
            price_raw = result["average" if alias == "avgcarprice" else "avg_price"]
