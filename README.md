...
промпты и секреты в лог не записываются. Поэтому по консоли можно убедиться, что полный
анализ действительно вызвал YandexGPT: один раз перед подтверждением и два или три раза
после подтверждения (рынок вызывается только при отсутствии API/fallback-цен).
