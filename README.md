...
Версия схемы записывается в `PRAGMA user_version`; миграция 2 добавляет test-mode
и поля котировок запчастей без удаления старых строк.
...
TTL-кэш и безопасный `UNAVAILABLE`-режим. Когда замена требует детали, но официальный
поставщик не настроен, статическая цена не подставляется и `BUY` блокируется.
Для реальной интеграции владелец должен предоставить официальную документацию и
настроить `PARTS_PROVIDER`, `PARTS_API_URL`, `PARTS_API_TOKEN`. Скрейпинг магазинов
и автоматические запросы к Drom не выполняются.
