...
- отдельные поля для полного текста объявления и повреждений, замеченных пользователем на фото;
- единый подробный коммерческий анализ рынка, ремонта, экономики, риска, ликвидности и торга;
...
| `YANDEXGPT_MODEL_URI` | Полный URI вызываемой модели. Значение по умолчанию — `gpt://b1gmpvdiu7blj491i69q/yandexgpt-5.1/latest`. |
...
записываются в журнал. Единый коммерческий промпт требует прямо помечать рыночные цены как
экспертную ориентировочную оценку, если у модели нет фактического доступа к объявлениям, и
запрещает утверждать, что модель видела конкретные карточки автомобилей.

При каждом обращении в консоль выводятся ID запроса, endpoint, модель, номер попытки,
HTTP-статус и `usage`, если его вернул API. Текст объявления, описание повреждений,
промпты и секреты в лог не записываются. Поэтому по консоли можно убедиться, что после
подтверждения единый полный коммерческий анализ действительно вызвал YandexGPT.

В актуальной версии подтверждение запускает один цельный коммерческий промпт
`prompts/full_analysis.txt`. Расчетные поля ответа не считаются доверенными: стоимость каждой
работы, бюджеты подготовки, прибыль, быстрая прибыль, ROI, безубыточная цена и требуемая скидка
повторно рассчитываются в Python. Отрицательный ROI сохраняет знак и не заменяется нулем.
...
prompts/full_analysis.txt       единый коммерческий промпт YandexGPT
config/fallback_prices.json     резервные рыночные цены
tests/                          автоматические тесты
```

В JSON-примере промпта литеральные фигурные скобки записываются как `{{` и `}}`,
