...
1. `/start`, выбор региона и постоянное кнопочное меню: «🚗 Новый анализ», «📋 История», «⚙️ Настройки», «ℹ️ Помощь».
...
8. Кнопка «📋 История» открывает расчёты без ввода ID; кнопка отчёта позволяет пересчитать цену без APIpoint и vision.

После `/start` команды не обязательны: назад, пропуск необязательных полей, отмена,
фотографии, история и действия отчёта доступны кнопками. Технические enum-коды
в интерфейсе заменяются русскими названиями.
...
цен деталей. Реализованы модели, протокол провайдера, медианная нормализация,
...
формирует структурированный запрос через Yandex Responses API и проверяет релевантность недоверенных карточек.
Для него отдельно задаются `YANDEX_PARTS_MODEL_URI`, `YANDEX_PARTS_PROMPT_VERSION`,
`YANDEX_PARTS_MATCH_PROMPT_VERSION` и `YANDEX_PARTS_MAX_RETRIES`. Промпт загружается
из `prompts/parts_query_v1.txt`, а ответ проходит строгую Pydantic-валидацию.
...
После предварительного отчёта кнопка «Добавить объявления» открывает ручной этап.
Можно передать 3–10 скриншотов для структурированного распознавания Yandex AI либо
3–10 строк `HTTPS-ссылка | видимое название | текущая цена`. Ссылки валидируются,
но не открываются ботом. Распознанные карточки показываются владельцу до подтверждения;
только после подтверждения Python-matcher и медианный нормализатор обновляют расчёт.
...
## Пользовательские настройки и актуальность цен

Кнопка `⚙️ Настройки` позволяет сохранить для владельца регион, целевую прибыль и
предпочтительное состояние запчастей (`Новые`/`Б/у`). Значения из `.env` остаются
значениями по умолчанию. `TEST_MODE` в Telegram только отображается и не переключается.
Миграция схемы 4 добавляет настройки пользователя без удаления существующих данных.

Генерация поисковой фразы и проверка карточек — два отдельных запроса Yandex AI с
разными строгими JSON-схемами и версионируемыми промптами. Если классификация Yandex
недоступна, применяется детерминированный matcher с отметкой `RULES_FALLBACK`;
автоматически найденные только этим fallback предложения не разрешают окончательный
`BUY`. Пользовательское подтверждение ручных карточек сохраняется отдельно.

Перед открытием истории и перерасчётом котировки проверяются относительно
`PARTS_PRICE_CACHE_TTL_HOURS`. Просроченная котировка получает `STALE`, экономика
перестраивается из структурированных данных, а окончательные показатели и `BUY`
скрываются до обновления цен.

## Воспроизводимость экономики

При создании расчёта в `scores.financial_settings` сохраняются фактически применённые
целевая прибыль, коэффициент быстрой продажи, фиксированные расходы и резерв. Ручное
подтверждение деталей, просмотр истории и изменение цены покупки используют этот
снимок, поэтому последующее изменение пользовательских настроек не меняет старый
расчёт. Для legacy-записи без снимка интерфейс явно сообщает о восстановлении параметров
из текущей конфигурации.

Полнота деталей проверяется по всем подтверждённым `RepairItem.requires_part` и их
стабильным `defect_id`. Для каждой котировки проверяются статус, TTL, деталь, количество
и состояние. Одна котировка из двух обязательных деталей никогда не считается полным
набором. Неизвестные доставка, наличие или состояние карточки не заменяются нулём,
`true` либо `NEW` и не участвуют в окончательной медиане.

Результат Yandex-классификации дополнительно проходит обязательные Python-правила:
явно неправильная сторона, положение, состояние или компонент вместо детали в сборе
отклоняются даже при ответе `EXACT`; `LIKELY` применяется только при достижении
`PARTS_MATCH_CONFIDENCE`. Ручной пользователь видит этот окончательный состав и причины
до нажатия «Подтвердить».
